...
import { Loader2, Check, ChevronsUpDown, Eye, Pencil, Trash2, MoreHorizontal, Copy, CheckCheck } from "lucide-react";
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
import { updateOrder, deleteOrder } from "@/services/orders.service";
...
const parseOrderedToArray = (ordered: string | string[]): string[] => {
    ...
    for (const item of MENU_ITEMS) {
        if (orderedLower.includes(item.label.toLowerCase()) || orderedLower.includes(item.value.replace(/_/g, ' '))) {
...
export function OrderActionDialog({
    ...
    React.useEffect(() => {
        if (!isViewMode && orderedItems && selectedDates) {
            const calculatedPrice = calculateTotalPrice(orderedItems, selectedDates.length);
    ...
    const handleCopyInvoice = async () => {
        ...
        try {
            ...
            const breakdown = getPriceBreakdown(parseOrderedToArray(order.ordered), order.dates.length);
            ...
            iframe.style.cssText = "position: fixed; left: -9999px; top: -9999px; width: 550px; height: 800px; border: none;";
    ...
    return (
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="border-2 border-black dark:border-white rounded-none shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
                    <DropdownMenuItem
                ...
                <AlertDialogContent className="border-2 border-black dark:border-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,1)] rounded-none bg-white dark:bg-black">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="text-base font-medium text-black/70 dark:text-white/70">
                            ...
                            <span className="font-bold text-black dark:text-white">{order.name}</span>?
